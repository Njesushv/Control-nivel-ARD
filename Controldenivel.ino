...
dist= float (0.0342*(tiempo/2));//Almacenamos el resultado de la operacion en la varible dist
mililitro= float (23-dist)*95;//Almacenamos en la variable mililitro el resultado de la resta de:(la distancia maxima que mide el sensor del deposito vacio) -(la distancia del sensor)*(los mililitros aprox que hay en un cm de altura) varia segun su contenedor
float distrue=23-dist;//Almacenamos en la variable distrue la dstancia maxima del deposito vacio menos la distancia, obteniendo asi la altura del liquido 
delay(1000);//delay de 600 milisegundos(Cada 600 milisegundos mandara una onda ultrasonica para actualizar la dist)
lcd.clear(); 
lcd.setCursor(0,0);
lcd.print("Estado:");  
Serial.println(dist);
